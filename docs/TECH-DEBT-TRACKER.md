...
- **Wave 4 root-doc cleanup & ILRepack documentation.**
  ...
  - Documented ILRepack disabled status in `Directory.Build.props` with rationale (double-merge risk, billing-blocked CI) and re-enable criteria.
...
- **ILRepack**: currently disabled (`ILRepackEnabled=false` in `Directory.Build.props`). Re-enable once CI billing is restored and a full `verify-local.ps1` pass succeeds with the flag set to `true`.
