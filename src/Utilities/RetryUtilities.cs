...
{
    /// <summary>
    /// Centralized retry and error handling utilities to eliminate code duplication.
    ...
    public static class RetryUtilities
    {
        ...
        public static bool IsRetryableException(Exception ex)
        {
            ...
            if (ex is HttpRequestException ||
                ...
                ex is NzbDrone.Common.Http.HttpException)
            {
                return true;
            }
            ...
            // Check for HttpException (from NzbDrone.Common.Http)
            if (ex is NzbDrone.Common.Http.HttpException httpEx)
            {
                // Parse status code from the exception if available
                if (httpEx.Response != null)
                {
                    return IsRetryableStatusCode(httpEx.Response.StatusCode);
                }
            ...
            string operationName = null)
        {
            ...
            while (true)
            {
                ...
                {
                    var opName = operationName ?? "operation";
                    Logger.Error(ex, $"All {maxRetries} attempts failed for {opName}");
                    throw;
                }
            }
        }
