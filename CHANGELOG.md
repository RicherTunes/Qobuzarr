...
- `ext/Lidarr.Plugin.Common` submodule re-pinned to **`682ddc3`** (`commonVersion` **`1.18.0-dev`**) — brings the `SimpleDownloadOrchestrator` naming + payload-validation seams the adoption above builds on (plus their adversarial-review hardening: thread-safety docs, OCE-contract + telemetry pins). `ext-common-sha.txt` matches the checked-out submodule HEAD (`CommonPinDriftTests`).
