...
- `ResumeHttpDownloader` now mirrors Common's SSRF-guard failure classification: DNS-resolution failures are retryable `HttpRequestException`s, while confirmed unsafe targets (literal private hosts or hosts resolving to private IPs) remain permanent `InvalidOperationException` blocks. This preserves the SSRF guarantee without turning a CDN DNS blip into a terminal track failure. (`QobuzDownloadClientResume416Tests`)
