...
- **A new `dependency-scan` CI job scans this repo's own dependency tree for known-vulnerable NuGet packages.** Added to `.gitea/workflows/ci.yml` (the primary Gitea gate) and mirrored in `.github/workflows/ci.yml` (reference-only mirror, guarded by `github.server_url == 'https://github.com'`), both invoking Common's `check-vulnerable-packages.ps1` against `Qobuzarr.sln` and, separately, `tests/Qobuzarr.Parity.Tests/Qobuzarr.Parity.Tests.csproj` (not part of the main solution, so it gets its own scan). A local run against both targets came back clean — no known-vulnerable packages, direct or transitive — so no package floors were needed in this repo for this pass.

### Fixed (API — CancellationToken threading, 2026-07-16)

- **A caller's `CancellationToken` can now actually stop `QobuzApiClient` requests and pagination loops.** `GetAsync`/`PostAsync` (and the internal `ExecuteRequestAsync`/`ExecuteCachedGetAsync`/`ExecuteUncachedAsync` pipeline) had no `CancellationToken` parameter, so every public method that accepted a token silently dropped it: `GetPlaylistAsync`, `GetTrackMetadataAsync`, `GetLabelAsync`, `GetArtistAsync`, `SearchPlaylistsAsync`, `SearchLabelsAsync`, and — worst — the unbounded pagination loops `GetArtistAlbumsAsync`, `GetLabelAlbumsAsync`, and `GetPlaylistTracksAsync`, which would keep issuing page requests after the caller had cancelled. `IQobuzApiClient.GetAsync`/`PostAsync` now take an optional `CancellationToken` (default preserves source compatibility for all existing call sites), the token is forwarded through the cached-GET executor (`CachingHttpExecutor.SendAsync`), the uncached POST path (`IQobuzHttpClient.ExecuteAsync`), and the session lookup, and each pagination loop checks the token between pages. Cancellation surfaces as `OperationCanceledException` and is logged at Debug (a caller decision), not Error. All `IQobuzApiClient` implementations updated (`AdaptiveQobuzApiClient` forwards the token into its rate-limiter wait; `BridgeQobuzApiClient` links it with the per-request timeout; CLI `SimpleQobuzApiClient` forwards to its HTTP client). (`QobuzApiClientCancellationTests`)

### Removed (dead code — TokenRefresher, 2026-07-16)

- **`src/Authentication/TokenRefresher.cs` (576 lines) + `ITokenRefresher` deleted.** Zero production wiring — nothing constructed, registered, or referenced them outside their own unit tests; live session renewal goes through Common's `StreamingTokenManager` + `SessionManager` stack. Removed the class, the interface, `TokenRefresherTests.cs`, and the now-orphaned `Token*` constants block in `QobuzarrConstants.Defaults` (`TokenRefreshBufferMinutes`, `TokenRefreshCooldownSeconds`, `TokenMaxRetryAttempts`, `TokenInitialRetryDelaySeconds`, `TokenCircuitBreakerThreshold` — all referenced only by the deleted class).

### Fixed (sweep hygiene — telemetry keys, double-counted ML stats, unguarded parse, 2026-07-10)
- **Performance metrics no longer key on the raw signed URL.** `QobuzHttpClient.ExecuteAsync` passed `request.Url.ToString()` to `IPerformanceMonitoringService.RecordApiCall`, so every call minted a unique, unbounded metric key (`request_ts`/`request_sig` vary per call) AND stored `request_sig`/`user_auth_token`/`app_id` values verbatim inside metric names. It now records the same coarse host+first-path-segment bucket the adaptive rate limiter already uses (`BuildRateLimitKey`), so metric cardinality is bounded per endpoint and no signed/secret query values reach metric names. (`QobuzHttpClientMetricKeyTests`)
- **`CompiledMLQueryOptimizer.RecordResult` no longer double-counts prediction telemetry.** It re-invoked `PredictComplexity`, whose `_statistics[result]++` / `_totalPredictions++` side effects counted the retrospective accuracy check as a new prediction, skewing `GetStatistics().Accuracy`. The decision-tree core is now a side-effect-free `ComputePrediction` that `RecordResult` calls directly; after 1 real prediction + 1 `RecordResult`, `TotalPredictions` is 1 (was 2). (`CompiledMLQueryOptimizerTests.RecordResult_DoesNotInflatePredictionTelemetry`)
- **`QobuzMetadataStrategy` no longer crashes the album loop on a non-numeric track ID.** Two unguarded `int.Parse(qobuzTrack.Id)` calls would throw `FormatException` on API data drift, failing the whole album; it now mirrors `LidarrMetadataStrategy`'s Wave-81 TryParse+warn+continue guard (skip the bad track, keep the rest). The classes are prod-unwired but tested. (`QobuzMetadataServiceCovTests.DownloadAlbumAsync_NonNumericTrackId_SkipsTrackAndContinues`)
- **Un-vacuumed the QobuzCLI compliance gates.** `SecurityComplianceTests`/`CodeQualityTests` located the "source root" by walking up until a directory contained a folder named `src`/`QobuzCLI` — which latched onto a bin-local `src/` folder created by the (since-removed) `ml-baseline-patterns.json` csproj Content item, so the security gates had silently scanned build output (i.e. nothing) forever and failed deterministically on CI's clean clone. `GetSourceRoot` now anchors on `Qobuzarr.sln` (throws if not found — no silent fallback); `Configuration_ShouldUseEnvironmentVariables` hard-asserts file existence and checks the real accessors (`QobuzIndexerSettings.GetAppId()/GetAppSecret()` falling back to `QOBUZ_APP_ID`/`QOBUZ_APP_SECRET`, empty `DefaultAppSecret` sentinel in `QobuzConstants`); `SourceCode_ShouldNotContainHardcodedCredentials` scans only repo-owned code (`src/`, `QobuzCLI/`; excludes `ext/`, `bin/`, `obj/`) with a tightened literal-assignment regex that ignores credential-slot *names* (env-var/config-key/header-name values) and is itself pinned by a positive/negative self-test so the gate can't go vacuous again. (`HardcodedCredentialDetector_SelfTest`)

### Removed (dead artifacts, 2026-07-10)
- **`src/Indexers/ml-baseline-patterns.json` (3.2 MB, ~160k lines) deleted and un-shipped.** It had zero runtime consumers (nothing ever loaded it — `QobuzConstants.MachineLearning.PatternsFileName` had no users and `IndexerMLManager` loads only `*.dll`), carried a stale 25-feature schema vs the live 16-feature extractor, yet `Qobuzarr.csproj` copied it to output and deployed it to every install. Removed the three csproj includes, the unused `PatternsFileName` constant, the file-probing test, and the stale doc/wiki/deploy-script claims that it was functional/shipped. Installs shrink by ~3.2 MB; the packaging-closure manifest (`packaging/expected-contents.txt`) never listed it, confirmed unchanged by the closure check.
- **`src/Indexers/SmartQueryCache.cs` (363 lines) deleted.** Zero references anywhere (src, QobuzCLI, tests — no test file covered it); only stale architecture-diagram nodes mentioned it, which were removed too.

### Fixed (download — bounded lifecycle-gate wait, 2026-07-09)
- **A leaked download-path lifecycle-gate lease can no longer wedge Lidarr's download-client threads forever.** `QobuzDownloadClient.AcquireDownloadPathLifecycleGateAsync` awaited its per-output-path `SemaphoreSlim` with no timeout and no `CancellationToken`, so a lease leaked by a wedged download task would hang every subsequent `Download()` or `RemoveItem(deleteData:true)` for that path indefinitely. The wait is now bounded (30s, matching the Race-1 `DownloadTask` cleanup budget) and cancellable; on timeout it throws a `TimeoutException` naming the contested output path (plus a Warn log) instead of hanging — the gate is never bypassed, since it guards the check→delete race. A timed-out `Download()` surfaces as a failed grab; a timed-out cleanup skips the delete (data left intact for a later removal) and completes without crash-looping. (`DownloadCleanupRaceTests`)

### Changed (download — adopt Common album-loop seams, 2026-07-09)
- `QobuzDownloadOrchestrator` no longer overrides the whole `DownloadAlbumAsync` album loop. Common's `SimpleDownloadOrchestrator` now exposes the two seams the override existed for — `BuildTrackOutputPath` (track file naming) and `ValidateDownloadedPayload` (post-download payload validation) — so qobuz shrinks to those two small overrides and shares the base album loop (bounded concurrency, AlbumCompletionPolicy incomplete⇒Failed, progress reporting) with tidal/amazon/apple. Behavior parity pinned by the existing `QobuzDownloadOrchestratorTests` (naming incl. multi-disc + curly apostrophe, text-payload rejection + cleanup, empty-stream-URL failure, SSRF redirect block).
...
- `ext/Lidarr.Plugin.Common` submodule re-pinned to **`6b17bcf`** (`commonVersion` **`1.18.0-dev`**) — brings the `SimpleDownloadOrchestrator` naming + payload-validation seams the adoption above builds on (plus their adversarial-review hardening: thread-safety docs, OCE-contract + telemetry pins). `ext-common-sha.txt` matches the checked-out submodule HEAD (`CommonPinDriftTests`).

### Fixed (download — SSRF guard transient-vs-permanent parity, 2026-07-03)
- `ResumeHttpDownloader` now mirrors Common's SSRF-guard failure classification: DNS-resolution failures are retryable `HttpRequestException`s, while confirmed unsafe targets (literal private hosts or hosts resolving to private IPs) remain permanent `InvalidOperationException` blocks. This preserves the retained helper's SSRF guarantee without turning a CDN DNS blip into a terminal track failure. (`QobuzDownloadClientResume416Tests`)
- The production Common-orchestrator download path now injects `SharedSystemHttpClient.MediaInstance`, a no-auto-redirect media client, so Common receives handled media redirects (301/302/303/307/308) and validates the target before any next-hop request is sent. This closes the qobuz-specific gap where the shared API/download client could auto-follow a compromised CDN redirect before Common's validator saw it, while Common `a5e9dca` keeps redirect-target DNS blips retryable. (`QobuzDownloadOrchestratorTests`, `TrackDownloadServiceOrchestratorTests`, `SharedSystemHttpClientDisposalTests`)
...
- `ext/Lidarr.Plugin.Common` submodule re-pinned to **`a5e9dca`** (`commonVersion` **`1.18.0-dev`**) — propagates the Common improvement wave into qobuz: SettingsBinder graceful-bind hardening, untested-primitive test coverage (RetryPolicy/SettingsBinder/UnicodeNormalizer), payload/rate-limit enforcement guards, template-scaffold fixes/gates, the shared `RemoteMediaUriGuard` transient-DNS classification fix, and redirect-target DNS/303 handling for no-auto-redirect media clients. `ext-common-sha.txt` matches the checked-out submodule HEAD (`CommonPinDriftTests`); qobuz's retained resume downloader now also treats unresolved media hosts as retryable while keeping private/unsafe targets hard-blocked. No qobuz API changes required.
