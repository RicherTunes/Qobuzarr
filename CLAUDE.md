...
**STILL UNPROVEN / KNOWN GAP — the Warning path is practically inert in production today.** The probe above only exercises the `Ok` branch (no Qobuz credentials were configured). Critically: the ONE indexer type Lidarr's live schema actually resolves is the classic host-native `QobuzIndexer`, wired to `IQobuzApiClient` → `AdaptiveQobuzApiClient` → `QobuzApiClient` — and **`QobuzApiClient.Gate` is hardcoded to always return `null`** (see its doc comment: "the gate lives on `BridgeQobuzApiClient`", a separate code path whose `CreateDownloadClientAsync` returns `null` and whose indexer adapter isn't what the live schema showed). Consequence: `QobuzAuthHealthCheck.Check()` will return `Ok` unconditionally on the currently-live wiring, no matter how badly auth is failing — **this health check cannot yet produce a Warning in production.** Closing that gap (wiring a populated `AuthFailureGate` onto the native `QobuzApiClient`/`QobuzIndexer` path, or an equivalent always-live auth signal) is a REQUIRED follow-up before this pilot delivers user-facing value; it was deliberately left out of this PILOT's scope (a behavior-changing edit to the live auth/HTTP pipeline, not a small adapter) and needs its own TDD + live validation.

**Live validation the lead still needs to run** (the part no container probe without real credentials can prove): configure a real Qobuz indexer with credentials that will fail (or force-expire a valid session), confirm a `Warning` entry sourced from `QobuzAuthHealthCheck` appears via `GET /api/v1/health` and on the Dashboard/System > Status banner, then re-authenticate and confirm it clears. Given the gap above, this will currently show `Ok` regardless of credential state until the native-path gate-wiring follow-up lands — that follow-up should be validated together with (or immediately before) this end-to-end check.
