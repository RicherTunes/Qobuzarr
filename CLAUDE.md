...
- `RemoteMediaUriGuard` / `UriGuardResult` — `src/Download/ResumeHttpDownloader.cs:39` validates provider stream URLs before the first request, while `MediaRedirectSafeSender` keeps the same guard across redirects. Since Common `3eee2f98`, DNS-resolution failures set `IsTransient` and route to retryable `HttpRequestException`; confirmed unsafe targets still hard-block with `InvalidOperationException`. Covered by `QobuzDownloadClientResume416Tests`.
