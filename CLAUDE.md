...
`src/Security/MetadataSanitizer.SanitizeVersion` is a thin Qobuz-local policy layer over Common's `MetadataFieldSanitizer.SanitizeVersion`. Its job is: (1) strip closed `<script>…</script>` blocks, (2) reject a small allow-list of genuinely-dangerous patterns by returning the safe default `"Version"`, then (3) delegate the music-domain text pipeline (control-char/zero-width stripping, FS-unsafe substitution, whitespace/length normalization) to Common. **The reject-to-`"Version"` behavior is a blunt instrument — only use it for patterns that can actually reach a hostile sink.**
...
- **File name segment** — those `GetFullTitle()` results flow into `TrackFileNameBuilder` → Common `FileSystemUtilities.CreateTrackFileName` → `Sanitize.FileNameSegment`, which replaces every filename-invalid char (incl. `|`) with a space and strips the rest. The final filename is always safe regardless of what survives version sanitization.
...
**Sibling sweep:** this over-broad dangerous-set is **qobuz-only**. tidalarr / amazonmusicarr / applemusicarr have no plugin-local `MetadataSanitizer` wrapper — they call Common's `MetadataFieldSanitizer.SanitizeVersion` directly, which already renders `|`→`_` in place and never nukes, so they never had this bug. No sibling fix needed; the reject-list is a legitimate qobuz-local policy (not a consolidation candidate unless a sibling later wants the same XSS/SQL defense-in-depth).
