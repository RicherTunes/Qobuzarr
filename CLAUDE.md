...
**Classifier-coverage gap — `FormatRestrictedByStreamingCredentials` (2026-07-05):** live-found HIGH re-grab loop — Ferry Corsten "Blueprint: Reprinted" re-grabbed 34× in 95 min, one track failing 96×, ~185GB class of waste. Root cause: `QobuzApiClient.ClassifyRestrictionReason` had no branch for the restriction `Code = "FormatRestrictedByStreamingCredentials"`, so it fell through to `TrackUnavailableReason.Unknown` — which is NOT in `IsPermanentlyUnavailable()` (only `Restricted`/`SubscriptionRestriction`), so terminal suppression never fired and `QobuzParser` kept emitting `ReleaseInfo` forever. Fix: this code is a subscription/credential-tier gate (the account's streaming credentials cannot fetch this format), identical in every quality-tier release for the catalog entry, so it now classifies as `SubscriptionRestriction` (permanent-with-interactive-recovery — withheld on automatic/RSS search, offered on interactive search for post-tier-upgrade recovery, exactly like the other subscription restrictions). Added to the `SubscriptionRestricted`/`FormatRestrictedBySubscription` matcher as an exact `EqualsRestriction(code, "FormatRestrictedByStreamingCredentials")` PLUS a broad `ContainsRestriction(code, "StreamingCredentials")` catch for sibling variants — the broad catch is provably safe because no known non-permanent Qobuz code contains `StreamingCredentials` (the purchase-credentials code is `...PurchaseCredentials` and is matched by the purchase branch first). Secondary quality-fallback question (does a LOWER format stream even when this format is credential-restricted?) investigated and deliberately NOT implemented: Qobuz already uses a distinct code `FormatRestrictedByFormatAvailability` for the genuine "lower format streams" case (routed to the soft-fallback path with a real URL via `QobuzStreamResponse.IsQualityFallbackOnly()`), and the live evidence (same track failing 96× deterministically) shows no usable stream is returned — the safety bias (bounded one-cycle suppression + interactive recovery beats an infinite loop) makes the permanent classification correct to ship. Pinned by `QobuzApiClientCovTests` (`GetStreamingInfoAsync_WithFormatRestrictedByStreamingCredentials_*`, `...WithStreamingCredentialsSiblingCode_*`, `...WithReasonOnlyStreamingCredentials_*`, plus over-reach guards `...WithPurchaseCredentialsCode_StaysRestricted_*` and `...WithFormatAvailabilityCodeButNoUrl_StaysUnknown_*`) and `QobuzDownloadClientTests.Download_WithFormatRestrictedByStreamingCredentials_RecordsReleaseSuppressionAndStillFails`.
...
## Per-track failure logging + sample-stream classification (2026-07-03) — Solar Fields diagnosability gap

**Problem this closes:** a real live album (Solar Fields "ORIGIN – Shaped By Time") re-grabbed 12x over ~3 hours, always failing "39/40 tracks • 1 failed" (`LogAlbumDownloadSummary`'s aggregate Info line). WHICH track and WHY was never logged at Info/Warn — only that one-line count. The per-track detail existed only at Debug (the retry warnings inside `GetStreamingInfoWithRetryAsync`) or nowhere at all (byte-download-stage failures happen inside Common's `SimpleDownloadOrchestrator`, whose `ILogger` `TrackDownloadService.BuildOrchestrator` never wires up — it passes `logger: null`, so that engine's own per-attempt/failure log lines are silently swallowed for Qobuz). By the time anyone looked at the loop, the Debug logs had rotated away and it was undiagnosable.

**Fix 1 — per-track Warn logging (`TrackDownloadService.LogPerTrackFailures`):** after computing the album summary, `DownloadAlbumAsync` now iterates every `!Success` entry in the orchestrator's `DownloadResult.TrackResults` and logs one Warn line per deficit track: `"Track failed: #{TrackNumber} '{Title}' (id {TrackId}) — reason={Reason}: {Message}"`. `{Reason}` is the `QobuzTrackClassifier`-recorded `TrackUnavailableReason` when one was recorded, or the literal string `"unclassified"` when it wasn't (the exact Solar Fields symptom — see Fix 2). `{Message}` is `trackResult.ErrorMessage` passed through `Lidarr.Plugin.Common.Security.Sanitize.SafeErrorMessage` (redacts URLs and auth-token-shaped substrings) before logging — defensive even though the orchestrator's own messages are already scrubbed at the source, since this is a second, independent log surface. This makes every deficit track diagnosable from Info-level logs alone, regardless of which stage (stream resolution vs. byte download) produced the failure, without touching the completion/suppression decision at all (pure logging addition).

**Fix 2 — sample-stream classification (`QobuzApiClient.GetStreamingInfoAsync`):** investigated WHY a deficit track can reach `AlbumDownloadException.TrackResults` with `Reason == null` (truly "unclassified", as opposed to a classified-but-generic `TrackUnavailableReason.Unknown`). One clean, evidenced gap: `streamingInfo.Sample == true` threw a raw `InvalidOperationException("Qobuz returned a sample stream; ...")` instead of the classified `TrackUnavailableException` seam the restriction branch immediately below it already uses. That raw exception reaches `TrackDownloadService.ResolveStreamAsync`'s generic `catch (Exception ex)`, which never calls `QobuzTrackClassifier.RecordSkipped` — so the track's `Reason` stays permanently `null`. A sample-only stream is semantically a preview: `QualityFallbackProvider.DetermineUnavailableReason` (a smaller, non-live-wired legacy classifier retained for its unit-coverage) already maps message text containing `"sample"`/`"preview"` to `TrackUnavailableReason.PreviewOnly` elsewhere in this codebase, so this now throws `TrackUnavailableException(trackId, message, TrackUnavailableReason.PreviewOnly)` — the same treatment restrictions get. `PreviewOnly` is deliberately **not** in `TrackUnavailableReasonExtensions.IsPermanentlyUnavailable` (only `Restricted`/`SubscriptionRestriction` are — see the terminal-suppression section above), so this classification **cannot** trigger terminal release suppression; it only fixes reporting/diagnosability (the track now logs `reason=PreviewOnly` via Fix 1 instead of `reason=unclassified`, gets a proper Warn instead of an Error, and — via the existing `GetIssuesSummary()`/`FormatGroupedFailureReasons` machinery — surfaces as `"1 preview-only"` in Lidarr's queue message instead of `"1 unspecified"`).

Deliberately scoped narrow: a track that Qobuz *always* serves as a sample fails **identically on every re-grab** (deterministic — the same track, the same reason, every time), which is the behavioral signature that actually matches the observed Solar Fields loop (12/12 cycles failing the same "1 track"), unlike a transient network blip which would be expected to flap. Two other unclassified-failure paths were found during this investigation but deliberately **left unclassified** (not fixed) because they are genuinely ambiguous or already retried, and forcing a specific `TrackUnavailableReason` onto them risks either mischaracterizing a real edition mismatch or, worse, an incorrect future suppression:
- `GetStreamingInfoAsync`'s `streamingInfo == null` and empty-`Url` branches (still raw `InvalidOperationException`) — could be a transient API hiccup or a genuine edition mismatch; no single safe classification fits both.
- Byte-download-stage failures inside `SimpleDownloadOrchestrator.DownloadViaUrlAsync` (already retried up to `MaxDownloadAttempts` for transient transport errors) and `QobuzDownloadOrchestrator`'s post-download payload-validation catch (currently a single-shot check with **no** retry, unlike the byte-copy loop) never reach the classifier at all — they happen after stream-URL resolution, a different code path than `ResolveStreamAsync`. These are transport/payload concerns, not catalog-availability concerns, so they don't belong in `TrackUnavailableReason` (which is specifically about Qobuz rights/availability gates); Fix 1's per-track logging now makes them visible as `reason=unclassified` with their real error text instead of silent, which is the diagnosability this task asked for. Closing the retry-coverage gap in the payload-validation catch is flagged as a follow-up, not done here — it's a resilience change, not a classification fix, and out of scope for a narrowly-evidenced, safety-biased change.

**Explicitly not done:** no broadening of terminal-suppression to cover unclassified or generic failures after N retries. The codebase's documented safety bias (see "Terminal release suppression" above) is that over-suppression (permanently hiding a recoverable album) is strictly worse than a bounded re-grab loop; an unclassified deficit continues to just fail-and-retry exactly as before, now with full Warn-level diagnosability.

Pinned by `TrackDownloadServiceOrchestratorTests` (`DownloadAlbumAsync_FailedTrack_LogsPerTrackWarnWithClassifiedReason`, `DownloadAlbumAsync_FailedTrackWithNoClassifiedReason_LogsUnclassified`, `DownloadAlbumAsync_SuccessfulTracks_DoNotEmitPerTrackFailureWarn`) and `QobuzApiClientCovTests.GetStreamingInfoAsync_WithSampleStream_ThrowsTrackUnavailableException_WithPreviewOnlyReason`.
...
## Metadata version sanitizer scope — shell-injection markers don't apply to the version field (2026-07-05)

`src/Security/MetadataSanitizer.SanitizeVersion` is a thin Qobuz-local policy layer over Common's `MetadataFieldSanitizer.SanitizeVersion`. Its job is: (1) strip closed `<script>…</script>` blocks, (2) reject a small deny-list of high-confidence active-content patterns by returning the safe default `"Version"`, then (3) delegate the music-domain text pipeline (control-char/zero-width stripping, FS-unsafe substitution, whitespace/length normalization) to Common. **The reject-to-`"Version"` behavior is a blunt instrument — only use it for patterns that can actually reach a hostile sink.**

**Sink reality (verified, do not re-litigate without re-checking):** the sanitized version string has exactly three ultimate sinks, none of which is a shell / `Process.Start` / raw-SQL / unescaped-HTML sink:
- **Indexer release title** — `TitleGenerator.GenerateQualitySpecificTitle` (`src/Indexers/Parsing/TitleGenerator.cs:51`) → `AlbumReleaseInfoBuilder` → `release.Title` (`src/Indexers/QobuzParser.cs:303`). A plain string Lidarr uses for release matching.
- **Album/track metadata title** — `QobuzAlbum.GetFullTitle` (`src/Models/QobuzAlbum.cs:181`) and `QobuzTrack.GetFullTitle` (`src/Models/QobuzTrack.cs:157`), consumed as `ReleaseInfo`/metadata title text (e.g. `QobuzIndexerAdapter.cs:190`).
- **File name segment** — those `GetFullTitle()` results flow into `TrackFileNameBuilder` → Common `FileSystemUtilities.CreateTrackFileName` → `Sanitize.FileNameSegment`, which replaces readable separators / invalid filename chars (incl. `|`) with spaces before handing off to `PathSegment` for any remaining OS-specific invalid-char filtering. The final filename is always safe regardless of what survives version sanitization.

**Consequence — shell-command-injection markers were removed from the version dangerous-set.** `&&`, `||`, `|`, `` ` ``, `$(`, `${` cannot reach a command interpreter here, so treating them as fatal was a pure false-positive that nuked legitimate editions (`"Puissance Sonore | Damn Beats Remix"`, `"53|01 Live Sessions"`, 36 live occurrences) to the literal `"Version"` — degrading metadata and raising edition-mismatch/import-collision risk. They are now neutralized **in place** instead: Common's pipeline maps `|`→`_` (and the plugin-local extra maps `&`→`_`), so `"Puissance Sonore | Damn Beats Remix"` → `"Puissance Sonore _ Damn Beats Remix"`, preserving the meaningful words while staying filename-safe. `` ` ``/`$` are valid filename chars and are simply preserved.

**Kept as defense-in-depth** (still return `"Version"`): XSS (`<script`, `javascript:`, `on*=` …), SQL indicators (`';`, `--`, `drop table` …), LDAP (`)(`, `(&`, `(|`), and XML (`<!DOCTYPE`, `SYSTEM"` …). These don't have a live sink today either, but they are cheap, don't false-positive on real editions, and guard against future sink additions. Only the shell-injection class was demonstrably harmful to real metadata, so only it was dropped (minimal change). Pinned by `MetadataSanitizerTests` (`SanitizeVersion_WithMusicPipeSeparator_*`, `SanitizeVersion_WithNumericPipeEdition_*`, `SanitizeVersion_WithShellCommandMarkers_NeutralizesInPlace_NotNuked`, plus the retained SQL/XSS/LDAP/XML `ShouldReturnSafeDefault` tests proving the kept patterns still fire).

**Sibling sweep:** this over-broad dangerous-set is **qobuz-only**. tidalarr / amazonmusicarr / applemusicarr have no plugin-local `MetadataSanitizer` wrapper and no current production caller of `MetadataFieldSanitizer.SanitizeVersion` in `src`, so they never had this reject-to-`"Version"` bug. If a sibling later adds version-field formatting, adopt Common's `MetadataFieldSanitizer.SanitizeVersion` directly and avoid a reject-to-default shell/operator list.
...
- `MetadataFieldSanitizer` — qobuz's `MetadataSanitizer.SanitizeVersion` delegates the universal music-metadata pipeline to Common, then applies the small plugin-local policy of returning `"Version"` only for high-confidence active-content markers (XSS/SQLi/LDAP/XML) and replacing `&` with `_`. Do not re-add shell/operator-only rejection (`|`, `&&`, `||`, backticks, `$()`): version strings are used as `ReleaseInfo.Title`, album/track metadata text, and filename segments via Common filename sanitization, not as shell, `Process.Start`, or raw SQL input. Those operators are legitimate music-edition punctuation and must be normalized in place so values like `"Puissance Sonore | Damn Beats Remix"` and `"53|01 Live Sessions"` keep their edition words.
...
- `RemoteMediaUriGuard` / `UriGuardResult` — production downloads route through Common's `SimpleDownloadOrchestrator` with `SharedSystemHttpClient.MediaInstance` (`src/Download/Services/TrackDownloadService.cs:207`), whose handler has auto-redirect disabled so Common validates handled media redirects (301/302/303/307/308) before the next request. The retained `ResumeHttpDownloader` helper (`src/Download/ResumeHttpDownloader.cs:39`) mirrors the same classification: since Common `a5e9dca`, DNS-resolution failures set `IsTransient` and route to retryable `HttpRequestException`; confirmed unsafe targets still hard-block with `InvalidOperationException`. Covered by `QobuzDownloadOrchestratorTests`, `TrackDownloadServiceOrchestratorTests`, `SharedSystemHttpClientDisposalTests`, and `QobuzDownloadClientResume416Tests`.
